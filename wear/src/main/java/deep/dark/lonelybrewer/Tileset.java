...
/**
 * Created by Dima on 2015/03/16.
 */
public class Tileset {

    private static final String TAG = "Tileset";
    private static final int SYMBOL_ROWS = 16, SYMBOL_COLS = 16,
            SYMBOL_COUNT = SYMBOL_ROWS * SYMBOL_COLS;

    public int symbolWidth, symbolHeight;
    private Rect[] symbolRects;

    public Tileset(Bitmap tilesetBitmap) {
        int width = tilesetBitmap.getWidth();
        int height = tilesetBitmap.getHeight();

        symbolWidth = width / SYMBOL_COLS;
        symbolHeight = height / SYMBOL_ROWS;

        symbolRects = new Rect[SYMBOL_COUNT];
        for (int index = 0, col, row, x, y; index < SYMBOL_COUNT; index++) {
            row = index / SYMBOL_COLS;
            col = index - row * SYMBOL_COLS;
            x = col * symbolWidth;
            y = row * symbolHeight;
            symbolRects[index] = new Rect(x, y, x + symbolWidth, y + symbolHeight);
        }
    }

    public Rect getSymbolRect (char symbol) {
        try {
            return symbolRects[CP437Table.indexOf((int) symbol)];
        }
        catch (Exception e) {
            Log.e(TAG, "didn't find " + symbol + " " + (int)symbol);
            return symbolRects[0];
        }
    }

    private static final List<Integer> CP437Table = Arrays.asList(
